        ...
        if( jsonData != null) {
            try {
                ...
                for (int i = 0; i < jsonRecords.length(); i++) {
                    ...
                    byte[] recordImage = android.util.Base64.decode(recordImageText, android.util.Base64.DEFAULT);
    ...
    public static String ReturnScore() {
        ...
        if (LevelAttributesStorage != null) {
            for (int i = 0; i < UserButtonInput.length(); i++) {
                ...
                //check if Userbuttoninput is a 1
                if (UserButtonInput.charAt(i) == '1') {
        ...
        if(ScoreNumber == 1) {
            scoreamountText = MessageFormat.format("you detected: {0} out of {1} sign", ScoreNumber, TotalSigns);
        }
        else
            scoreamountText = MessageFormat.format("you detected: {0} out of {1} signs", ScoreNumber, TotalSigns);
